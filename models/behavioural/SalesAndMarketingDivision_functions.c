...
int SMD_strategise() {
  ...
  if (iteration_loop % 20 == 1) {
    ...
    for (s=0; s<MARKETING_FACTOR_COUNT; s++) {
      ...
      for (i=1; i<(HISTORY.size); i++) {
        ...
        scores[s] += ( 1.0 / pow(FACTOR_ALPHA, i) ) *
          ...
          ( (1.0 - FACTOR_BETA * HISTORY.array[history_index - 1].factor_proportions[s]) ) );
      }
    }
    ...
    for (s=0; s<MARKETING_FACTOR_COUNT; s++) {
      ...
      factor_cost = FACTORS[s].available * FACTORS[s].cost;
      FUNDS -= factor_cost;
      CUMULATIVE_COSTS += factor_cost;
    }
...
int SMD_apply_factors() {
  ...
  for (int m=0; m<MARKETING_FACTOR_COUNT; m++) {
    ...
    FACTORS[m].available -= daily_factors[m];
  }
